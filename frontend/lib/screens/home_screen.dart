...
// Helper function to check if mood is negative
bool _isNegativeMood(String? moodKeyword) {
const negativeMoods = ['sad', 'angry'];
return moodKeyword != null && negativeMoods.contains(moodKeyword);
}
...
final isNegativeMood = _isNegativeMood(moodProvider.todaysMoodKeyword);
...
// 6. MOOD SUPPORT WIDGET (Only shows for negative moods)
if (isNegativeMood) ...[
const SizedBox(height: 16),
_FadeInSlide(
animation: _fadeAnimation,
delay: 0.32,
child: _buildMoodSupportWidget(context),
),
],
...
Widget _buildMoodSupportWidget(BuildContext context) {
return Container(
width: double.infinity,
...
Row(
children: [
const Icon(Icons.psychology_outlined, color: Colors.blue, size: 24),
...
const SizedBox(height: 12),
const Text(
"Sorry to hear you're feeling down. Try brain games or mental exercises to lift your mood!",
  style: TextStyle(
    fontSize: 14,
    color: Colors.blue,  // Use bracket notation instead
    height: 1.4,
  ),
),
const SizedBox(height: 16),
...
const SizedBox(width: 16),
TextButton(
onPressed: () => Navigator.pushNamed(context, '/color-game'),
style: TextButton.styleFrom(
foregroundColor: Colors.blue.shade700,
),
child: const Row(
